...
def analyze_batch_results(master_batch_folder_path):
    ...
    # Gather all CSVs + inject parameter value
    ...
    for metric in metric_columns:
        fig = px.line(combined_df, x=param_name, y=metric,
                      color="Dataset", line_dash="Model", markers=True,
                      title=f"{metric} vs {param_name.capitalize()} (All Synthesizers + Models)")
    ...
    # Export Excel: One sheet per metric
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        ...
        for metric in metric_columns:
            pivot_df = combined_df.pivot_table(index=[param_name], columns=["Dataset", "Model"], values=metric)
            pivot_df.to_excel(writer, sheet_name=metric, index=True)
