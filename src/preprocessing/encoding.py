...
def encode_categorical_features(original_data, target_column, test_data=None):
    """
    ...
    - test_data (DataFrame, optional): The test dataset to transform using the same encoder.

    Returns:
    - cleaned_train (DataFrame): Transformed training dataset with binary encoding.
    - cleaned_test (DataFrame or None): Transformed test dataset if provided.
    ...
    if categorical_cols:
        encoder = ce.BinaryEncoder(cols=categorical_cols, drop_invariant=True, handle_unknown='ignore')
