...
def run_analysis(run_folder_path, analysis_config, logger=None):
    # Check if analysis is enabled
    if not analysis_config.get("enable_analysis", True):
        print("⚠️ Singular analysis skipped (disabled in config).")
        if logger:
            logger.info("Singular analysis skipped (disabled in config).")
        return
    ...
    if not os.path.exists(result_csv):
        ...
        if logger:
            logger.error(f"model_performance.csv not found in: {run_folder_path}")
        return
    ...
    if logger:
        logger.info(f"Metrics detected: {metric_columns}")
    ...
    if logger:
        logger.info(f"Saved raw results: {raw_copy_path}")
    ...
    if logger:
        logger.info("Singular run analysis complete.")
