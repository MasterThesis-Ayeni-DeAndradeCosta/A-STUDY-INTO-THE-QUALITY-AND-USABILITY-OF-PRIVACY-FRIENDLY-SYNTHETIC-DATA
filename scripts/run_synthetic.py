...
def run_synthetic(cleaned_data, dataset_name, target_column, output_dir, config, logger=None):
    ...
    for synth_name, synthetic_data in synthetic_datasets.items():
        ...
        if logger:
            logger.info(f"[SYNTHETIC PIPELINE] Evaluating synthetic dataset generated by {synth_name}...")
            real_cols = list(cleaned_data.columns)
            synth_cols = list(synthetic_data.columns)
            meta_cols = list(metadata.to_dict().get("columns", {}).keys())

            logger.info(f"[SYNTHETIC EVAL] Real data columns     ({len(real_cols)}): {real_cols}")
            logger.info(f"[SYNTHETIC EVAL] Synthetic data columns ({len(synth_cols)}): {synth_cols}")
            logger.info(f"[SYNTHETIC EVAL] Metadata columns      ({len(meta_cols)}): {meta_cols}")
            logger.info(f"[SYNTHETIC EVAL] Shape — Real: {cleaned_data.shape}, Synthetic: {synthetic_data.shape}")

            real_set = set(real_cols)
            meta_set = set(meta_cols)
            missing_in_meta = real_set - meta_set

            if missing_in_meta:
                logger.warning(f"[SYNTHETIC EVAL] Columns in real data missing from metadata: {missing_in_meta}")

            logger.info("[SYNTHETIC EVAL] Metadata structure:\n" + json.dumps(metadata.to_dict(), indent=2))
